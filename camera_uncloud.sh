...
-p,  --password    Specify SSH password. (Requres sshpass)
     --force-ssh   Don't use sshpass even if you have it installed. Ignores the -p option.
EOF
}
...
if [[ ! -z $(dpkg -s sshpass 2>/dev/null) ]] && [[ "$forceSSH" = false ]]; then
	sshpass -p $PASSWORD ssh admin@${CAMERA} /bin/bash "$command"
else
	ssh admin@${CAMERA} /bin/bash "$command"
fi
